...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    // user: '18295790529@163.com',
    // 这里密码不是qq密码，是你设置的smtp授权码
    pass: 'foxyxamgunidbebf'
    // pass: 'rrcqwtbjexlibadd'
  }
})
...
function sendMail(message, projectName, zipName) {
  ...
  const mailOptions = {
    ...
    to: projectName.indexOf('dev') !== -1 ? maillistDev : maillistProd, // 接收者
    subject: `${customerName}打包文件`, // 主题
    html: `<h4>Dear:</h4>
    ...
    attachments: [
      {
        filename: `${customerName}${moment().format('YYYY-MM-DD')}.tar.gz`,
        path: zipFile
      }
    ]
  }
